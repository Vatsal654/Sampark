 ...
 * playwright.cors.config.ts, playwright.alert.config.ts, and
 * playwright.location.config.ts (also excluded here) each start their
 * own dedicated mock server alongside `next dev`, which this config's
 * single production webServer can't provide.
 */
import { defineConfig, devices } from '@playwright/test';

export default defineConfig({
  testDir: './e2e',
  testIgnore: ['dev-mode.spec.ts', 'cors.spec.ts', 'alert-submission.spec.ts', 'location-sharing.spec.ts'],
  fullyParallel: true,
